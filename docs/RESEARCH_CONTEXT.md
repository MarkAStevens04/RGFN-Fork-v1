...
- [ ] Run RGFN through the **active-learning loop** on the validated 6TD3 oracle. ⚠️ Inner-loop learning confirmed (1 round, exp `009`); multi-round + true-oracle labels still pending a compute-node run (login-node CPU cap killed the query-batch docking). A **fast GPU-docking oracle for sEH** now exists (`DockingSEHOracle`, exp `010`, 0.60 s/mol over 300 molecules) — fast enough to drive the multi-round loop to completion on the standard RGFN benchmark target via `experiments/active_learning/seh/submit_al_seh.sh`.
...
- [ ] Ablation: **pose selection — CNN vs. Vina** — does keeping the top-Vina pose discriminate as well as the CNN-selected pose? Since the differential is pure Vina, CNN pose-picking is gnina's *only* remaining job, so this is effectively a go/no-go on gnina: **if CNN selection is unnecessary, drop gnina for QuickVina2-GPU** (~1.57→0.60 s/mol, entry 010) and port the 6TD3 oracle to GPU docking. **Scripts ready, awaiting Balam** (exp `008`): `sbatch experiments/ablations/pose_selection/submit_pose_ablation.sh` (smoke-test off-Balam first: `python experiments/ablations/pose_selection/smoke_test.py`).
...
| [008](../Logs/008_pose-selection-cnn-vs-vina.md) | 2026-06-25 | 6TD3 — does pose selection (CNN vs Vina) change discrimination? | *Pending Balam run* — scripts ready; go/no-go on gnina (CNN's only job is pose-picking): if Vina-selected poses discriminate as well, **drop gnina for QuickVina2-GPU** (entry 010, 0.60 s/mol) |
