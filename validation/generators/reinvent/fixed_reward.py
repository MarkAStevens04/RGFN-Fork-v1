"""Frozen reward generators for the REINVENT 4 entrant.

Per the repo convention this is a **per-adapter copy** (mirrors ``validation/generators/{rxnflow,
fraggfn,s3gfn}/fixed_reward.py``) so the ``reinvent4`` env stays self-contained. The classes are
deliberately the same two the other entrants use, with the same numbers, because the whole point of
the benchmark is that every generator optimizes the **same** frozen reward:

- :class:`SEHFrozenReward` — the Bengio-2021 sEH MPNN. Our benchmark's sEH proxy IS this model
  (``rgfn/gfns/reaction_gfn/proxies/seh_proxy.py`` loads the same ``bengio2021flow`` weights), so
  REINVENT is optimizing the identical function RGFN/SCENT/RxnFlow/S3-GFN do.
- :class:`DRD2FrozenReward` — the cached TDC DRD2 activity oracle (``oracle/drd2_current.pkl``),
  with TDC's exact count-Morgan/FCFP6 featurization.

**What REINVENT uses, and what it does not.** :meth:`predict` returns the RAW oracle value — the
scale our per-target mode gate is defined on (sEH ``> 7.0``, DRD2 ``> 0.5``) — and it is the only
method the scoring component calls. The sibling adapters also expose ``reward() = exp(clip(v))``
because a GFlowNet needs a positive unnormalized reward; REINVENT does **not** work that way. It
maps each component to ``[0, 1]`` with a declarative ``transform`` in the run TOML and combines
components with its own DAP objective, so shaping belongs in the config where a reader can see it,
not hidden in this file. ``reward()`` is kept only for interface parity with the siblings and is
never called on the REINVENT path.
"""
