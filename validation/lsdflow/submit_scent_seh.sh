#!/bin/bash
#SBATCH --job-name=lsdflow_scent_seh
...
#
# CHECKPOINT: use the recipe-logging re-run (experiments/fixed_reward/scent_seh/submit_fixed_scent_seh_recipes.sh)
...
#      validation/lsdflow/results/scent_seh_pilot/
...
MODE_REWARD=${MODE_REWARD:-7.0}                 # sEH "hit" bar; matches the RGFN headline (Logs/026)
...
# CKPT: 'auto' (default) discovers the newest scent_seh run's checkpoint — after the recipe re-run
...
CKPT=${CKPT:-auto}
if [ "$CKPT" = "auto" ]; then
    CKPT=$(ls -td "$SCRATCH"/rgfn_runs/experiments/fixed_reward/scent_seh/*/train/checkpoints/last_gfn.pt 2>/dev/null | head -1)
    echo "[auto] discovered newest scent_seh checkpoint: ${CKPT:-<NONE FOUND>}"
fi
if [ -z "$CKPT" ] || [ ! -f "$CKPT" ]; then echo "ERROR: no scent_seh checkpoint found; set CKPT=..."; exit 1; fi
CFG=${CFG:-validation/configs/scent_seh_fixed.gin}
OUT_DIR=${OUT_DIR:-$SCRATCH/rgfn_runs/lsdflow/scent_seh_${SLURM_JOB_ID}}
...
python -m validation.lsdflow.harness.run \
    ...
    --run-id "scent_seh_${SLURM_JOB_ID}" \
    --out-dir "$OUT_DIR"

echo "DONE lsdflow_scent_seh -> $OUT_DIR"
