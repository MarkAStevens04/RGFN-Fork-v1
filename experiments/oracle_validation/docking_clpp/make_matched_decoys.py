#!/usr/bin/env python
"""Property-matched (DUD-E-style) decoys for the ClpP docking-threshold calibration.

The negative set the ClpP Vina cutoff must be calibrated against: drug-like molecules
matched to the known ClpP binders' physicochemistry (MW / logP / HBD / HBA / rotatable
bonds / formal charge) but a **different scaffold** (ECFP4 Tanimoto < 0.35 to every
active), so the docking cutoff can't win on gross properties alone. Identical machinery
to the sEH version (`../seh_proxy/make_matched_decoys.py`); by default it **reuses the
already-fetched drug-like ChEMBL background pool** (`../seh_proxy/background_pool_chembl.csv`,
~30k molecules — target-agnostic), so no second 30k fetch is needed.

Writes `data/validation-molecules/ClpP_decoys_matched.csv` (curated, committed).

Run in the `rgfn` env (needs internet ONLY if the shared background cache is absent):
...
def clean_smiles(smiles: str) -> str | None:
    """Largest organic fragment, uncharged, canonical (matches fetch_clpp_actives)."""
    if not smiles:
        return None
...
def fetch_background(cache: Path, mw_lo: int, mw_hi: int, bin_w: int, per_bin: int) -> list[str]:
    ...
    if cache.exists():
        ...
        print(f"[bg] loaded {len(smi)} cached background SMILES from {cache}", file=sys.stderr)
        return smi
    seen: set[str] = set()
...
def main() -> None:
    ...
    ap.add_argument(
        "--actives",
        type=Path,
        default=REPO_ROOT / "data" / "validation-molecules" / "ClpP_actives_chembl.csv",
    )
    ap.add_argument(
        "--out",
        type=Path,
        default=REPO_ROOT / "data" / "validation-molecules" / "ClpP_decoys_matched.csv",
    )
    # Reuse the shared, target-agnostic drug-like background pool by default.
    ap.add_argument(
        "--cache", type=Path, default=HERE.parent / "seh_proxy" / "background_pool_chembl.csv"
    )
