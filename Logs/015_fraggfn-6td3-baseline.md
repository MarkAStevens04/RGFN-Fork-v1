...
The science answer: **the non-synthesizable baseline generates molecules that dock as real glues, competitively with RGFN** — best differential −4.86 (vs RGFN's −4.7 in entry 011), median −2.06 (vs ≈−2.4), 54% of candidates beating the −2.0 glue cutoff (vs 64%), all fully diverse (32 distinct scaffolds per 32-molecule round, novelty 1.0). It also reproduces RGFN's two known weaknesses — large molecules (MW ≈720) and low QED (≈0.15). So on raw oracle score FragGFN is in the same league as RGFN; the differentiator RGFN must lean on is **synthesizability** (FragGFN's molecules have no route), not glue-score superiority. One caveat: this used the GPU oracle (AUROC ≈0.88) while the RGFN reference (entry 011) used the CPU gnina-search oracle (≈0.95), and the matched-oracle RGFN GPU run (entry 014, job 69481) died on a wedged docking node — so a clean *same-oracle* head-to-head still needs a healthy RGFN GPU rerun.
...
- **Matched-oracle head-to-head:** rerun RGFN on the *same* GPU oracle on a healthy node (entry-014, job 69481, died on a wedged OpenCL node) so FragGFN-GPU vs RGFN-GPU is apples-to-apples — the current comparison is FragGFN-GPU vs RGFN-CPU (entry 011). Then put both per-round `dvina` distributions + Top-K on one plot.
...
- RGFN reference: entry 011 (job 69445); the matched-oracle RGFN GPU run that failed is entry 014 (job 69481, `al_6td3_gpu-*.out`).
...
**Head-to-head vs RGFN.** Equal seed `D_0`, budget (3×32, top-16), β=8, and proxy architecture.

| | FragGFN (this run, 69482) | RGFN reference |
|---|---|---|
| best `dvina` | **−4.86** | −4.7 (entry 011) |
| median `dvina` | −2.06 | ≈ −2.4 (entry 011) |
| frac ≤ −2.0 | 52/96 (54%) | 61/96 (64%) (entry 011) |
| diversity / scaffolds | 0.87 / all distinct | diverse (entry 011) |
| MW / QED drift | MW ≈720, QED ≈0.15 | same drift (entry 011/009) |
| synthesizable route | **none** (`has_route=0`) | yes, by construction |
| oracle used | GPU `Docking6TD3GpuOracle` (AUROC ≈0.88) | CPU `Docking6TD3Oracle` (≈0.95) (entry 011) |

**Caveat on the comparison.** The cleanest baseline-comparison row would be RGFN on the *same* GPU oracle (entry 014, job 69481) — but that run **died at docking on a wedged OpenCL node** (round 1: "Docking attempt #1–10 failed on GPU 0", `|D|=408 (+0)`; with no abort guard yet it continued to round 2 on an unchanged `D`). That failure is what motivated the all-NaN abort guard now mirrored into the FragGFN loop. FragGFN's run happened to land on a healthy balam009 and docked 32/32 every round. So the numbers above compare FragGFN-GPU to RGFN-CPU (entry 011); a matched RGFN-GPU rerun on a healthy node is the remaining piece.
